...
## Nota desta fase (estruturação, repositório local)

Não há recursos Azure provisionados nesta fase (ver Anexo C). As integrações com **Azure AI Search** (busca de chunks) e **Azure OpenAI** (embedding + completion) são definidas como **interfaces** (`SearchService`, `CompletionService`) e implementadas em tasks posteriores; até lá são injetadas como stubs para teste. A primeira task (setup do endpoint + validação de input) não depende de nenhum serviço Azure e é testável isoladamente.
